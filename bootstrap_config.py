...
def build_config_yaml(env_choice: str, risk_pct: str, max_pos: str) -> str:
    """Build config.yaml content for given env_choice (demo/live/testnet). Used by main() and tests."""
    burn_in_phase = "demo" if env_choice == "demo" else ("live_small" if env_choice == "live" else "testnet")
    return f"""# Generated by bootstrap_config.py
mode: paper
...
def main() -> None:
    ...
    burn_in_phase = "demo" if env_choice == "demo" else ("live_small" if env_choice == "live" else "testnet")
    config_content = build_config_yaml(env_choice, risk_pct, max_pos)
    config_path = config_dir / "config.yaml"
    config_path.write_text(config_content, encoding="utf-8")
    print(f"Wrote {config_path}")

    print("\nDone. Validate: python3 run_bot.py validate")
    if env_choice == "demo":
        print("Demo burn-in ready. Start: ./scripts/start_testnet_burnin.sh (or python3 run_bot.py run)")
    else:
        print("Guarded live: when ready run ./scripts/check_small_live_ready.sh then ./scripts/start_small_live.sh")
