...
11. **Demo automation**: The optional Demo orchestration (`automation.enabled`, `automation.demo_orchestration_enabled`) only runs readiness/evaluation/optimizer/shadow and writes recommendation artifacts under `artifacts/automation/`. It never auto-promotes configs or environments; the operator must still run `python run_bot.py promote --config-id ...` and `python run_bot.py promote-env --confirm-live` manually when ready. A **systemd timer** (`money-flow-momentum-automation.timer`) can run `python run_bot.py automation cycle` periodically (e.g. every 15 min); the timer does not start the trading bot and does not auto-promote. Trading and orchestration are separate services.
